...
function vnet.Watch(typ, cbk, ops)
    ...
    if callbacks[typ] and not override then
        local info = debug.getinfo(callbacks[typ][1]);
        error("a callback is already registered for this type in file " .. info.short_src .. " at line " .. info.linedefined .. "; perhaps you meant to override?")
    end
